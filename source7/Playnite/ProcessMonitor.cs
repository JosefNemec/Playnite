...
public class ProcessMonitor : IDisposable
{
    ...
    public async Task WatchUwpApp(string familyName, bool alreadyRunning)
    {
        ...
        var failCount = 0;
        var matchProcString = familyName.Replace("_", @"_.+__", StringComparison.Ordinal);
        ...
        while (true)
        {
            ...
            try
            {
                ...
                {
                    if (process.TryGetMainModuleFilePath(out var procPath))
                    {
    ...
    {
        SyncContext.Post((_) => TreeStarted?.Invoke(this, new TreeStartedEventArgs { StartedId = processId }));
    }

    private void OnTreeDestroyed()
    {
        SyncContext.Post((_) => TreeDestroyed?.Invoke(this, EventArgs.Empty));
    }
}
